...
def get_spread_doc(doc):
    affected,detail = get_affected_as(analyzer.links,analyzer.elems,"104.244.42.0/24","136168")
...
def get_path_change_doc(doc):
    ...
    def callback(attr, old, new):
        ...
        time = slider.value
        try:
            diffs = get_edges_from_paths(detail[time])
        except:
            return
...
def get_updates_doc(doc):
    asn = "44356"
    cnts = get_packets_count(analyzer.elems,1,asn)
    timestamps = []
    ys = []
    for cnt in cnts:
    ...
    def update_data(attr, old, new):
        ...
        cnts = get_packets_count(analyzer.elems,sample_length,asn)
        timestamps = []
        ys = []
        for cnt in cnts:
            timestamps.append((cnt[0]+cnt[1])/2)
